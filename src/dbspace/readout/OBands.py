...
class naive_readout:
    def __init__(self, feat_frame, ClinFrame):
        ...
        self.all_feats = ["L-" + band for band in bands] + [
            "R-" + band for band in bands
        ]
    ...
    def sig_bands(self, stats="ks", weeks=["C01", "C24"]):
        ...
        for pt in self.pts:
            for ff in self.bands:
                print("Computing " + " " + ff)
                _, self.ks_stats[pt][ff], self.week_distr[pt][ff] = (
                    self.feat_frame.scatter_state(
                        weeks=weeks,
                        pt=pt,
                        feat=ff,
                        circ=self.circ,
    ...
    def all_ensemble_change(self, plot=True):
        ...
        for ff, freq in enumerate(self.all_feats):
            pre_flat_list.append(
                [item for sublist in self.pre_feat_vals[:, ff] for item in sublist]
            )
            post_flat_list.append(
                [item for sublist in self.post_feat_vals[:, ff] for item in sublist]
            )
    ...
    def get_ensemble_change(self, band, plot=True):
        ...
        return {
            "diff": (np.mean(pre_flat_list) - np.mean(post_flat_list)),
            "var": (np.var(pre_flat_list) - np.var(post_flat_list)),
            "ks": stat_check,
        }
...
class OBands:
    ...
    def patient_stacks(self, pt_list):
        ...
        for pt in pt_list:
            pt_recordings = [
                recording
                for recording in self.BRFrame.file_meta
                if recording["Patient"] == pt
            ]
            pt_stack = {}
            for week in weeks:
                pt_stack[week] = [
                    recording["FeatVect"]
                    for recording in pt_recordings
                    if recording["Phase"] == week
                ]
            pt_stacks[pt] = pt_stack
    ...
    def mean_psds(self, patients="all", weeks=["C01", "C23"]):
        ...
        for pt in patients:
            ...
            for ww in weeks:
                all_psds = [
                    ...
                    for rr in fmeta
                    if rr["Phase"] == ww and rr["Patient"] == pt
                ]
        ...
        for ch, chann in enumerate(["Left", "Right"]):
            ...
            w0psds = np.squeeze(
                np.array(
                    [
                        10 * np.log10(val[:, ch, :].T)
                        for key, val in week_avg.items()
                        if key == weeks[0]
                    ]
                )
            )
    ...
    def day_vs_nite(self):
        ...
        for light in sleeps:
            Circ[light] = [
                [
                    [
                        (rr["FeatVect"][feat]["Left"], rr["FeatVect"][feat]["Right"])
                        for feat in dbo.feat_order
                    ]
                    for rr in fdnmeta
        ...
        dn_var = np.array(
            [[pt_day_nite[pt][feat]["Var"] for feat in range(10)] for pt in self.do_pts]
        )
        ...
        plt.figure()
        plt.pcolormesh(
            (P_val < (0.05 / 10)).astype(np.float32), cmap=plt.get_cmap("Set1_r")
        )
        plt.colorbar()
    ...
    def c_vs_c(self, early, late):
        ...
        for state in weeks:
            diff_states[state] = [
                [
                    ...
                    for rr in fdnmeta
                    if rr["Patient"] == pt and rr["Phase"] == state
                ]
                for pt in self.do_pts
            ]
        # night_recs = [[[(rr['FeatVect'][feat]['Left'],rr['FeatVect'][feat]['Right']) for feat in dbo.feat_order] for rr in fdnmeta if rr['Patient'] == pt and rr['Circadian'] == 'night'] for pt in self.do_pts]
        ...
        for pp, pt in enumerate(self.do_pts):
            pt_two_states[pt] = stats.ranksums(
                np.array(pt_two_states[state[0]][pp]),
                np.array(pt_two_states[state[1]][pp]),
            )
    ...
    def compare_states(
        ...
        feats["Left"] = [
            (dispfunc(rr["FeatVect"][feat]["Left"]), rr["Phase"])
            for rr in fdnmeta
            if rr["Patient"] in pt and rr["Phase"] in weeks
        ]
        feats["Right"] = [
            (dispfunc(rr["FeatVect"][feat]["Right"]), rr["Phase"])
            for rr in fdnmeta
            if rr["Patient"] in pt and rr["Phase"] in weeks
        ]
        ...
        for cc, ch in enumerate(["Left", "Right"]):
            weekdistr = {
                swap_key[week]: [a for (a, b) in feats[ch] if b == week]
                for week in weeks
            }
            if stat == "ks":
                outstats[ch] = stats.ks_2samp(
                    weekdistr[swap_key[weeks[0]]], weekdistr[swap_key[weeks[1]]]
                )
            elif stat == "ranksum":
                outstats[ch] = stats.ranksums(
                    weekdistr[swap_key[weeks[0]]], weekdistr[swap_key[weeks[1]]]
                )
            elif stat == "t":
                outstats[ch] = stats.ttest_1samp(
                    weekdistr[swap_key[weeks[0]]], weekdistr[swap_key[weeks[1]]]
                )
    ...
    def scatter_state(
        ...
        for cc, ch in enumerate(["Left", "Right"]):
            weekdistr = {
                week: [a for (a, b) in feats[ch] if b == week] for week in weeks
            }
            #
            if stat == "ks":
                ...
                outstats[ch] = stats.ttest_1samp(
                    weekdistr[weeks[0]], weekdistr[weeks[1]]
                )
            ...
            for cc, ch in enumerate(["Left", "Right"]):
                ...
                if plot_type == "scatter":
                    ...
                    ser1 = np.array(
                        [
                            fd
                            for fd, weeek in zip(feat_data, week_data)
                            if weeek == weeks[0]
                        ]
                    )
                    ser2 = np.array(
