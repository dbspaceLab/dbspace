...
class head_model:
    ...
    def plot_model(self):
        ...
        EEG_Viz.plot_coords(display_vox_loc,active_mask=[True]*display_vox_loc.shape[0],color=(1.,0.,0.)) # First, we plot the tracts from the DTI
...
def DTI_support_model(pt,voltage,dti_parcel_thresh=70,eeg_thresh=70,condit='OnT'):
    ...
    y_translate = np.zeros_like(display_vox_loc)
    y_translate[:,1] = 1
    display_vox_loc += brain_offset * z_translate + tract_offset * y_translate #DISPLAYED
...
#VISUALIZATION SANITY CHECK PASSES!!!! SO WTF STUPID SHIT IS HAPPENING TO SCREW UP PRIMARY LOCS?
    if 0:
        ...
        EEG_Viz.plot_coords(parcel_coords,active_mask=primary_locs,color=(0.,1.,0.),plot_overlay=False,alpha=0.8)
#%%
    ...
    primary_locs = np.logical_and(primary_locs,second_normed < 0.2) #this worked for 906
    #%%    
    # Find First order EEG channels
    ...
    second_chann_mask = np.logical_and(second_chann_mask == 1, ~(primary_chann_mask == 1)).astype(np.int)
    
    #pdb.set_trace()
    #%%
    #Channel mask writing
    EEG_support = {'primary':primary_chann_mask,'secondary':second_chann_mask,
                   'parcel_coords':parcel_coords,'primary_locs':primary_locs,'eeg_scale':eeg_scale,'EEG_coords':EEG_coords,
                   'second_locs':second_locs,'dti_scale_factor':dti_scale_factor,
                   'brain_offset':brain_offset,'tract_offset':tract_offset,'display_vox_loc':display_vox_loc}
    #pickle.dump(EEG_support,open('/tmp/' + pt + '_' + condit + '_' + voltage,'wb'))
    return EEG_support
...
if __name__=='__main__':
    for pt in ['906']:
        for voltage in [4]:
            supp_model = DTI_support_model(pt,str(voltage),dti_parcel_thresh=15,eeg_thresh=60)
