...
def plot_3d_scalp(
    ...
    if unwrap:
        ...
        if anno_top:
            sc_top = plt.scatter(
                flat_etrodes[annotate_list, 0],
                flat_etrodes[annotate_list, 1],
                c=band[annotate_list],
                vmin=clims[0],
                vmax=clims[1],
                s=300,
                cmap=cm,
        ...
        if binary_mask:
            ...
            bin_mask = plt.scatter(
                flat_etrodes[ch_ones, 0],
                flat_etrodes[ch_ones, 1],
                c="black",
                vmin=0,
                vmax=1,
                s=300,
...
def plot_coords(
    band, active_mask=[], rad=[], color=[0.0, 0.0, 0.0], plot_overlay=True, alpha=0.8
):
    if active_mask == []:
...
def plot_tracts(band, active_mask=[], rad=[], color=[0.0, 0.0, 0.0], alpha=1):
    if not rad:
        rad = np.zeros_like(band[:, 2])
        rad[active_mask] = 20
